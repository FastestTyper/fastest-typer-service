...
CMD java -jar -Dspring.profiles.active=${profile} fastest-typer-service-0.0.1-SNAPSHOT.jar --dbHost=${dbHost} --dbHost=${dbPort} --dbUser=${dbUser} --dbPassword=${dbPassword} --dbInitializationMode=${dbInitializationMode} --dbDdlAuto=${dbDdlAuto} --rabbitHost=${rabbitHost} --rabbitPort=${rabbitPort} --rabbitUser=${rabbitUser} --rabbitPassword=${rabbitPassword}
